...
jQuery(document).ready(function($) {
    ...
    var whr = (wd - cowd) / 2;
    var hg = $('.section-breadcrumbs').height();
    var tp = parseInt(hg) - 40;

    var svhg = $('.service-detail').height();
    var hgwd = $(window).height();
    var sthg = parseInt(hg) + svhg - hgwd;

    var sbwd = $('.section-sidebars').width();

    if (wd > 991) {
        $(window).on("scroll",function() {
            if ($(this).scrollTop() > tp) {
                $('.section-sidebars').css({'width': sbwd, 'position': 'fixed', 'top': '90px', 'right': whr, 'z-index': '9999'});
            }

            if ($(this).scrollTop() > sthg) {
                $('.section-sidebars').css({'position': 'absolute', 'top': 'auto', 'bottom': '0', 'right': '15px'});
            }

            if ($(this).scrollTop() < sthg) {
                $('.section-sidebars').css({'position': 'fixed', 'top': '90px', 'bottom': 'auto', 'right': whr});
            } 

            if ($(this).scrollTop() < tp) {
                $('.section-sidebars').css({'position': 'initial'});
            }
        });
    }
